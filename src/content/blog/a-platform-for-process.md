...
Not because any one part of it was impossible to understand, but because the system is large enough that knowing one part does not mean you understand the whole. There are models, views, actions, reports, permissions, workflows, modules, computed fields, inherited methods, scheduled jobs, accounting rules, inventory flows, and a lot of business logic hidden behind ordinary-looking buttons.

That makes the early learning process strange. You can build things before you fully understand the system they are joining. Sometimes that works fine. Other times, you solve a problem in a way that technically works but does not fit the platform very well.

The hard part is developing intuition. Over time, you start to learn when to extend an existing model, when to create a new one, when to lean on Odoo’s workflow, and when a customization is fighting the platform instead of using it.

That intuition is expensive. It comes from reading code, tracing methods, breaking things, fixing them, and slowly realizing that almost nothing in an ERP is isolated.

## What the ERP Gives You

The value of Odoo is not that every module is perfect.

The value is that it gives you a base layer of business software: accounting, sales, inventory, payroll, reports, permissions, documents, portals, and process. Some of those areas may only be “decent” for a particular client, but decent matters when the alternative is building all of that from scratch, or paying 7 different services a high subscription cost.

An ERP gives you enough of everything that custom development can focus on what the company actually needs to be great at.
...
The ERP is not just a database with screens. It's a high-level opinion about what software should look like as a whole.
...
For example, while I'd feel fine running SQL to update a random record or field, I would be very hesitant to run raw SQL against Odoo data without a lot of research (especially if it involves accounting), because the system is complex enough that looking at the database doesn't tell the full story.
...
That is what makes it useful, and also what makes it difficult.
...
That is probably the best version of the tradeoff: an ERP gives you a working shape for the business, but you have to learn when to follow that shape and when to carefully bend it.
